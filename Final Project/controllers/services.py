...
if not OPENAI_API_KEY:
    ...
    try:
        ...
        # For development: Disable SSL verification to work around Windows certificate issues
        # WARNING: This is NOT secure for production! Only use in development.
        try:
            http_client = httpx.Client(verify=False, timeout=60.0)
            client = OpenAI(api_key=OPENAI_API_KEY, http_client=http_client)
            logger.info("OpenAI client initialized (SSL verification disabled for development)")
        except Exception as e:
            logger.error(f"Failed to initialize OpenAI client with custom HTTP client: {str(e)}")
        ...
        # If httpx not available, use default client
        try:
            client = OpenAI(api_key=OPENAI_API_KEY, timeout=60.0)
        except Exception as e:
...
def infer_subject_from_topic_ai(topic, full_message=""):
    ...
    This is more flexible than hardcoded keywords and can handle edge cases.
    """
    if not topic:
        return None
    ...
    try:
        prompt = f"""
        Given the following educational topic, infer what subject it belongs to.
        
        Topic: "{topic}"
        Full message context: "{full_message}"
        
        Return ONLY the subject name. Common subjects are:
        - Math (or Mathematics)
        - Science
        - English Language Arts (or ELA, English, Reading, Writing)
        ...
        If the topic is clearly about math (numbers, calculations, equations, decimals, fractions, etc.), return "Math".
        If the topic is clearly about science (experiments, nature, physics, chemistry, biology, etc.), return "Science".
        If the topic is clearly about language/reading/writing, return "English Language Arts".
        If the topic is clearly about history/government/society, return "Social Studies".
        
        If you cannot determine the subject with confidence, return "null".
        
        Return ONLY the subject name, nothing else.
        """
        
        if not client:
            logger.error("OpenAI client is not initialized. Check OPENAI_API_KEY in .env file.")
            return None
        
        response = client.chat.completions.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "You are a helpful assistant that identifies educational subjects from topics. Return only the subject name."},
        ...
        if subject.lower() in ['null', 'none', 'unknown', '']:
            return None
        ...
        if 'math' in subject_lower or 'mathematics' in subject_lower:
            ...
            return "Science"
        elif any(word in subject_lower for word in ['english', 'ela', 'language arts', 'reading', 'writing']):
            return "English Language Arts"
        elif any(word in subject_lower for word in ['social studies', 'history']):
            return "Social Studies"
        
        return subject.strip()
    except Exception as e:
        logger.error(f"Error inferring subject from topic using AI: {str(e)}")
        return None
...
def generate_lesson_plan_with_ai(data):
    ...
    try:
        ...
        if not client:
            logger.error("OpenAI client is not initialized. Check OPENAI_API_KEY in .env file.")
            return "<p>I apologize, but the AI service is not properly configured. Please check that your OpenAI API key is set in the .env file.</p>"
        ...
        try:
            ...
            logger.error(f"OpenAI API error generating lesson plan: {error_msg}", exc_info=True)
            
            # Provide more helpful error messages
            if "Connection" in error_msg or "timeout" in error_msg.lower():
                return "<p>I apologize, but I'm having trouble connecting to the AI service right now. This could be due to a network issue or the service being temporarily unavailable.</p><p><strong>Please try again in a moment.</strong> If the problem persists, check your internet connection or contact support.</p>"
            elif "rate limit" in error_msg.lower() or "quota" in error_msg.lower():
                return "<p>I apologize, but the AI service is currently experiencing high demand. Please wait a moment and try again.</p>"
            elif "api key" in error_msg.lower() or "authentication" in error_msg.lower():
                return "<p>I apologize, but there's an issue with the AI service configuration. Please contact support.</p>"
            else:
                return f"<p>I apologize, but I encountered an error while generating your lesson plan: {error_msg}</p><p>Please try again or contact support.</p>"
        
    except Exception as e:
        logger.error(f"Unexpected error generating lesson plan: {str(e)}", exc_info=True)
        return f"<p>I apologize, but I encountered an unexpected error while generating your lesson plan: {str(e)}</p><p>Please try again or contact support.</p>"
...
def generate_worksheet(data):
    ...
    try:
        ...
        if not client:
            logger.error("OpenAI client is not initialized. Check OPENAI_API_KEY in .env file.")
            return "<p>Sorry, the AI service is not properly configured. Please check that your OpenAI API key is set in the .env file.</p>"
        ...
        try:
            ...
            error_msg = str(api_error)
            logger.error(f"OpenAI API error generating worksheet: {error_msg}")
            if "Connection" in error_msg or "timeout" in error_msg.lower():
                return "<p>Sorry, I'm having trouble connecting to the AI service right now. Please try again in a moment.</p>"
            else:
                return f"<p>Sorry, I couldn't generate the worksheet at this time: {error_msg}</p>"
    except Exception as e:
...
def generate_unit_plan_with_ai(data):
    ...
    try:
        ...
        try:
            ...
            error_msg = str(api_error)
            logger.error(f"OpenAI API error generating unit plan: {error_msg}", exc_info=True)
            if "Connection" in error_msg or "timeout" in error_msg.lower():
                return "<p>Sorry, I'm having trouble connecting to the AI service right now. Please try again in a moment.</p>"
            else:
                return f"<p>Sorry, I couldn't generate the unit plan: {error_msg}</p>"
    except Exception as e:
...
def process_conversation(user_message, state):
    ...
    # CRITICAL: ALWAYS use grade level from user_profile if available - NEVER ask for it
    ...
    if saved_grade and str(saved_grade).strip() and saved_grade != 'NOT SET' and saved_grade != '':
        # Always populate grade_level from user profile if it's not already set or if it's different
        if not data.get('grade_level') or data.get('grade_level') != str(saved_grade).strip():
            data['grade_level'] = str(saved_grade).strip()
            state['data'] = data
            logger.info(f"✓ Using saved grade level from user profile: '{saved_grade}'")
    
    # If we're at GRADE_LEVEL step but have a saved grade, skip to COLLECTING_BASIC
    if step == ConversationState.GRADE_LEVEL and data.get('grade_level'):
        step = ConversationState.COLLECTING_BASIC
        state['step'] = step
        logger.info(f"✓ Skipping GRADE_LEVEL step - using saved grade: '{data.get('grade_level')}'")
        ...
        if value and value != 'null':
            ...
            if isinstance(value, str) and value.strip():
                ...
                if key not in data or value != 'Not specified':
                    # For topic, preserve user's original text (don't auto-capitalize)
                    if key == 'topic':
                        # Use the original user message if it contains the topic, otherwise use extracted value
                        user_lower = user_message.lower()
                        topic_lower = value.lower()
                        # Try to find the topic in the original message to preserve formatting
                        if topic_lower in user_lower:
                            # Find the original text from user message
                            start_idx = user_lower.find(topic_lower)
                            if start_idx != -1:
                                data[key] = user_message[start_idx:start_idx+len(value)].strip()
                            else:
    ...
    if step == ConversationState.GRADE_LEVEL:
        # CRITICAL: ALWAYS check user_profile first - this is the source of truth from Google sign-in
        user_profile = state.get('user_profile', {}) or {}
        saved_grade = user_profile.get('grade_level')
        
        # If we have a saved grade from profile, ALWAYS use it - NEVER ask
        if saved_grade and str(saved_grade).strip() and saved_grade != 'NOT SET' and saved_grade != '':
            data['grade_level'] = str(saved_grade).strip()
            state['data'] = data
            state['step'] = ConversationState.COLLECTING_BASIC
            logger.info(f"✓ GRADE_LEVEL step: Using saved grade from profile: '{saved_grade}'")
            return {
                'success': True,
                'state': state,
        ...
        # Check if we got the grade level from message (fallback)
        if data.get('grade_level'):
            ...
            # This should rarely happen if onboarding is working correctly
            # But if it does, we still need to ask (though this indicates a bug in onboarding)
            logger.warning(f"GRADE_LEVEL step reached without grade - user_profile: {user_profile}, data: {data}")
            return {
                'success': True,
                'state': state,
        ...
        # CRITICAL: Ensure grade_level is always set from user_profile if not already in data
        if not data.get('grade_level'):
            saved_grade = user_profile.get('grade_level')
            if saved_grade and str(saved_grade).strip() and saved_grade != 'NOT SET' and saved_grade != '':
                data['grade_level'] = str(saved_grade).strip()
                state['data'] = data
                logger.info(f"✓ COLLECTING_BASIC: Populated grade_level from user_profile: '{saved_grade}'")
        
        # Check if we have subject and topic
        # First, try to infer subject from topic if we have a topic but no subject
        if not data.get('subject'):
            # Try to get subject from extraction first
            if extracted.get('subject'):
                data['subject'] = extracted['subject']
            else:
                # Try to infer from topic if we have one
                topic_to_check = extracted.get('topic') or data.get('topic')
                if topic_to_check:
                    inferred_subject = infer_subject_from_topic_ai(topic_to_check, user_message)
                    if inferred_subject:
                        data['subject'] = inferred_subject
                        logger.info(f"Inferred subject '{inferred_subject}' from topic '{topic_to_check}' using AI")
            ...
            if not data.get('subject') and user_message and str(user_message).strip() and not any(k in user_message.lower() for k in KEYWORDS_DONT_KNOW + KEYWORDS_GENERATE_NOW):
        ...
        # Update topic if we extracted one (preserve user's original text)
        if extracted.get('topic') and extracted['topic'] != data.get('subject'):
            # Use the original user message to preserve formatting
            user_lower = user_message.lower()
            topic_lower = extracted['topic'].lower()
            if topic_lower in user_lower:
                # Find the original text from user message to preserve capitalization
                start_idx = user_lower.find(topic_lower)
                if start_idx != -1:
                    # Use original text from user message, but try to get the full phrase
                    # Look for the full user message if it's similar to the topic
                    if len(user_message) - len(extracted['topic']) < 20:  # If user message is close to topic length
                        data['topic'] = user_message.strip()
                    else:
                        data['topic'] = user_message[start_idx:start_idx+len(extracted['topic'])].strip()
                else:
                    data['topic'] = extracted['topic'].strip()
            else:
                # If topic not found in user message, use extracted but preserve user's text if it's similar
                if topic_lower in user_lower or user_lower in topic_lower:
                    data['topic'] = user_message.strip()
                else:
                    data['topic'] = extracted['topic'].strip()
        elif not data.get('topic') and user_message.strip():
            # If no topic extracted but user provided a message, use it as topic
            # This handles cases where extraction might miss the topic
            user_lower = user_message.lower()
            # Check if it's not a question or command
            if not any(k in user_lower for k in KEYWORDS_DONT_KNOW + KEYWORDS_GENERATE_NOW + ['what', 'how', 'when', 'where', 'why']):
                data['topic'] = user_message.strip()
                logger.info(f"Using user message as topic: '{data['topic']}'")
        
        # Check if user is repeating the same topic (loop detection)
        existing_topic = data.get('topic', '').lower().strip()
        user_msg_lower = user_message.lower().strip()
        
        # Check if user is repeating the topic or the system's confirmation message
        is_repeating = (
            existing_topic and 
            (user_msg_lower == existing_topic or 
             user_msg_lower in existing_topic or 
             existing_topic in user_msg_lower or
             f"okay, {data.get('grade_level', '').lower()} {existing_topic}" in user_msg_lower or
             user_msg_lower.startswith('okay') and existing_topic in user_msg_lower)
        )
        
        if is_repeating and existing_topic:
            # User is repeating the topic - they want to proceed
            # Don't ask again, just proceed to follow-up questions
            logger.info(f"User repeated topic '{data['topic']}', proceeding to follow-up questions")
        elif not data.get('topic'):
            # If we don't have a topic yet, ask for it
            return {
                'success': True,
                'state': state,
        ...
        if (extracted_intent.get('subject') or extracted_intent.get('topic')) and not any(phrase in user_lower for phrase in KEYWORDS_MODIFICATION):
            ...
            # If we have topic but no subject, try to infer subject from topic
            if new_data.get('topic') and not new_data.get('subject'):
                inferred_subject = infer_subject_from_topic_ai(new_data['topic'], user_message)
                if inferred_subject:
                    new_data['subject'] = inferred_subject
                    logger.info(f"Inferred subject '{inferred_subject}' from topic '{new_data['topic']}' in ASK_FOR_MODIFICATIONS using AI")
            ...
            # If we have topic (and possibly inferred subject), proceed to follow-up questions
            elif new_data.get('topic'):
                # Make sure we have subject before proceeding
                if not new_data.get('subject'):
                    return {
                        'success': True,
                        'state': state,
                        'message': f"Got it. What subject is this for?"
                    }

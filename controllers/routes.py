...
def register_routes(app, login_manager, mail_instance=None):
    ...
    def chat():
        ...
        try:
            ...
            # Get user profile data to enhance conversation processing
            user_profile = None
            try:
                users_collection = db.get_collection('users')
                user_doc = users_collection.find_one({'id': current_user.id})
                if user_doc:
                    ...
                    # Add user profile to state for use in processing
                    state['user_profile'] = user_profile
            except Exception as e:
                logger.warning(f"Could not load user profile for enhanced parsing: {str(e)}")
    ...
    def export_pdf(conversation_id):
        ...
        try:
            ...
            title_text = f"{lesson_plan.get('grade_level', '')} {lesson_plan.get('subject', '')} - {lesson_plan.get('topic', '')}"
